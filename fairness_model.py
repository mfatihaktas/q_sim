...
def ff_ETh_ub(har, t, sdist_m):
  ...
  ET = ES + (har/2)*ES2/(1 - har*ES)
  return ET if ET >= 0 or ET < ET_UB else None
...
def ff_har_ub(car, t, sdist_m):
  ...
  ES = float(mpmath.quad(lambda s: Pr_Sgs(s), [0, 10000*10] ) )
  return 1/ES - 0.05
...
def ff_ETh_approx_(har, car, t, sdist_m):
  ...
  V22 = X_n_k(V, 2, 2)
  # Pr_V22_l_X = mpmath.quad(lambda x: V22.cdf(x)*X.pdf(x), [X.l_l, X.u_l] )
  Pr_V_l_X = mpmath.quad(lambda x: V.cdf(x)*X.pdf(x), [X.l_l, X.u_l] )
  def Pr_Sgs(s):
    # a = V22.tail(s)*Pr_V22_l_X + 1 - Pr_V22_l_X
    a = V22.tail(s)*Pr_V_l_X + 1 - Pr_V_l_X
    return V.tail(s)*V22.tail(s)**(t-m) * (a*(1 - ro) + ro)**m
  
  ES = mpmath.quad(lambda s: Pr_Sgs(s), [0, 10000*10] )
  ES2 = mpmath.quad(lambda s: 2*s*Pr_Sgs(s), [0, 10000*10] )
...
def ff_ETh_approx(har, car, t, sdist_m):
  ...
  def ESk_i(k, i): # kth moment of Si
    return mpmath.quad(lambda s: k*s**(k-1) * (1 - Si_cdf(i, s)), [0, 10000*10] )
  ...
  def EWk_i(k, i): # kth moment of Wi
    ...
    Y = Exp(i*car)
    p = mpmath.quad(lambda v: Si_cdf(t-m+i, v)*Y.pdf(v), [0, Y.u_l] ) # Pr{S_{t-m+i} < Exp(i*car) }
